...
(function() {
    ...
    Render.world = function(render) {
        ...
        if (options.bodyNames)
            Render.bodyNames(render, bodies, context);
     ...
     * @param {RenderingContext} context
     */
    Render.bodyNames = function(render, bodies, context) {
        var c = context,
            i,
            j;
        ...
        for (i = 0; i < bodies.length; i++) {
            ...
            var parts = bodies[i].parts.filter((part) => part.label == "Circle Body");
            for (j = parts.length > 1 ? 1 : 0; j < parts.length; j++) {
                var part = parts[j];
                c.font = "14px Arial";
                c.fillStyle = 'rgba(255,255,255,0.5)';
                c.fillText(part.atomType + part.atomState, part.position.x - 8.5, part.position.y + 5);
            }
        }
    };
